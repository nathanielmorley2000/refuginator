monteCarlo <- function(entry, decline, duration, nit, summary) {
  ...
  for(it in 1:nit){
    vec = floor(stats::runif(nrow(summary), min = 0, max = summary$localities_with_data + 1)) # Randomly generate integer for each time bin between 0 and the number of available localities
